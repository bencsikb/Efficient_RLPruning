...
def test(data,
    ...
    training = not called_directly
    #training = False
    #device = select_device('cuda', batch_size=batch_size)
    ...
    if not training:
        ...
        #path = data['test'] if opt.task == 'test' else data['val_images']  # path to val/test images
        val_img_path = data['val_img']
        val_label_path = data['val_label']
        print("valpath ", val_img_path)
        print(val_label_path)
        dataloader = create_dataloader(val_img_path, val_label_path, imgsz, batch_size, 32, opt,
