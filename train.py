...
def train(hyp, opt, device, tb_writer=None, layer_to_prune=None, alpha=None):
    ...
    #layer_to_prune = 0
    #alpha = 1
    prune_network(model, yolo_layers, layer_to_prune, alpha)
    model.cuda()
    base_lr = 0.0005
    optimizer = torch.optim.Adam(model.parameters(), lr=base_lr, weight_decay=1e-3)
    for g in optimizer.param_groups:
        g['lr'] = base_lr
    scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=opt.epochs, eta_min=0.0001, last_epoch=-1)
    scheduler.step()
...
if __name__ == '__main__':
    ...
    if opt.local_rank == -1 or ("RANK" in os.environ and os.environ["RANK"] == "0"):
        check_git_status()
    """
    opt.weights = '/home/blanka/YOLOv4_Pruning/runs/exp257_coco128_orig/weights/last.pt'
    ...
    if not opt.evolve:
        ...
        train(hyp, opt, device, tb_writer, layer_to_prune, alpha)
